...
- **Platform-split corrections (with manual override):** auto-detects iOS vs Android, or force it with the in-app **Platform toggle** (Auto / iOS / Android). Android locks exposure (or meters ISO×shutter); iOS applies dim-end suppression to its HDR-boosted preview and uses photo EXIF metering as the true reading.
- **⚙️ Universal calibration (collapsed, works on any reading):** get a live or photo reading, enter the current UV from a reference — a weather app works — and the app rescales to match, remembered forever on-device. One point = scale; two points (bright + shade) = slope+offset. The current reading is shown right next to the input for direct comparison.
- **Photo-handoff hardening:** the live camera pauses *before* the iOS picker opens (freeing the hardware so iOS can't break the handoff), a 20-second guard stops camera auto-restart from swallowing a photo result, the live camera auto-resumes if you cancel, and a session flag detects iOS page reloads mid-pick.
- **Exposure-lock anchoring:** when Android locks exposure, the pre-lock metered reading pins the pixel scale per device — live and photo paths agree by construction. **HEIC photos from iPhones are supported** (embedded EXIF parsed directly, unit-tested).
- **📸 Photo meter (the accurate iOS path):** separate **Take sky photo** (opens the camera directly) and **Upload sky photo** (gallery/files) buttons — the picked image appears instantly as a thumbnail with its ISO/shutter/aperture metrics, so you can see the upload worked. SunD names exactly which EXIF fields are missing if a photo lacks them, and applies the incident-meter equation `E = C·N²/(t·ISO)` with **platform-tuned constants** (Android C≈1200, iOS C≈200 — phone AE exposure targets differ ~5× in bright sun, so one textbook constant fits neither), making iPhone/iPad and Android readings directly comparable. iOS live video stays available as a rough guide only (Apple HDR tone-maps the preview).
