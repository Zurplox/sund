...
- **Platform-split corrections (with manual override):** auto-detects iOS vs Android, or force it with the in-app **Platform toggle** (Auto / iOS / Android). Android locks exposure (or meters ISO×shutter); iOS applies strong dim-end suppression to its HDR-boosted preview for the live view and uses photo EXIF metering as the true reading. A 20-second guard stops the page's camera auto-restart from swallowing a photo result when the iOS native camera closes.
