...
- **⚙️ Live calibration (iOS, collapsed by default):** enter the current UV from any reference — a weather app works — once (remembered forever on-device) or twice (bright sun + shade); the iOS live curve is rescaled/interpolated from those anchors. The built-in default is a piecewise-linear curve fitted from real field reports.
