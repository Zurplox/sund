...
- **📸 Photo meter (the accurate iOS path):** snap a photo of the sky — SunD reads ISO, shutter speed and aperture straight from the photo's EXIF and applies the same incident-meter equation as Android (`E = C·N²/(t·ISO)`), so iPhone/iPad and Android readings are directly comparable. iOS live video stays available as a rough guide only (Apple HDR tone-maps the preview, so its absolute brightness is unreliable by design).
